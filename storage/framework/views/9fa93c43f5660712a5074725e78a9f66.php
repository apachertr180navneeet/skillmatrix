

<?php $__env->startSection('content'); ?>

<!-- Start Hero Area -->
    <section id="hero-area" class="hero-area style3">
        <!-- <img src="assets/images/startup-shape.png" alt="#" class="custom-shape"> -->
        <!-- Single Slider -->
        <div class="hero-inner hero-inner2">
            <div class="container">
                <div class="row ">
                    <div class="col-lg-12 co-12">
                        <div class="home-slider">
                            <div class="hero-text hero-text2">
                                <h1 class="wow fadeInUp" data-wow-delay=".5s"
                                    style="visibility: visible; animation-delay: 0.5s; animation-name: fadeInUp;">Terms
                                    and Conditions</h1>



                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
        <!--/ End Single Slider -->
    </section>
    <!--/ End Hero Area -->
    <!-- terms & condition page content & design -->

    <!-- Start Terms Condition Section -->
    <section class="terms-condition-section">
        <div class="container">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
                    <div class="terms-condition-content">

                        <div class="terms-block">
                            <h2>1. Introduction</h2>
                            <p>Welcome to PrecureSkill. By accessing or using our platform, you agree to comply with and
                                be bound by these Terms & Conditions. If you do not agree, please do not use our
                                services.</p>
                        </div>

                        <div class="terms-block">
                            <h2>2. Services</h2>
                            <p>PrecureSkill provides a structured employee training system including:</p>
                            <ul>
                                <li>Training content (videos, SOPs, checklists)</li>
                                <li>Skill development modules</li>
                                <li>Performance tracking tools</li>
                                <li>Assessment and evaluation systems</li>
                            </ul>
                        </div>

                        <div class="terms-block">
                            <h2>3. User Responsibilities</h2>
                            <p>By using our platform, you agree to:</p>
                            <ul>
                                <li>Provide accurate information</li>
                                <li>Use the platform only for lawful purposes</li>
                                <li>Not misuse, copy, or distribute content without permission</li>
                                <li>Maintain confidentiality of login credentials</li>
                            </ul>
                        </div>

                        <div class="terms-block">
                            <h2>4. Account Access</h2>
                            <ul>
                                <li>Users are responsible for maintaining the security of their accounts</li>
                                <li>Any activity under your account is your responsibility</li>
                                <li>We reserve the right to suspend or terminate accounts if misuse is detected</li>
                            </ul>
                        </div>

                        <div class="terms-block">
                            <h2>5. Intellectual Property</h2>
                            <p>All content on PrecureSkill (training material, design, software) is the property of
                                PrecureSkill and is protected by applicable laws.</p>
                            <p>You may not copy, reproduce, or distribute without written permission.</p>
                        </div>

                        <div class="terms-block">
                            <h2>6. Payments & Subscriptions</h2>
                            <ul>
                                <li>All payments made for services are non-refundable unless stated otherwise</li>
                                <li>Subscription plans must be renewed as per selected terms</li>
                                <li>Prices may change with prior notice</li>
                            </ul>
                        </div>

                        <div class="terms-block">
                            <h2>7. Limitation of Liability</h2>
                            <p>PrecureSkill is not liable for:</p>
                            <ul>
                                <li>Any direct or indirect business loss</li>
                                <li>Performance outcomes solely dependent on user implementation</li>
                                <li>Technical issues beyond our control</li>
                            </ul>
                        </div>

                        <div class="terms-block">
                            <h2>8. Data & Privacy</h2>
                            <p>We respect your data and handle it according to our Privacy Policy.</p>
                            <p>We do not sell your personal data to third parties.</p>
                        </div>

                        <div class="terms-block">
                            <h2>9. Service Availability</h2>
                            <p>We aim to provide uninterrupted service but do not guarantee 100% uptime.</p>
                            <p>Maintenance or updates may temporarily affect access.</p>
                        </div>

                        <div class="terms-block">
                            <h2>10. Termination</h2>
                            <p>We reserve the right to suspend or terminate access if:</p>
                            <ul>
                                <li>Terms are violated</li>
                                <li>Misuse or fraudulent activity is detected</li>
                            </ul>
                        </div>

                        <div class="terms-block">
                            <h2>11. Changes to Terms</h2>
                            <p>PrecureSkill may update these Terms & Conditions at any time.</p>
                            <p>Continued use of the platform means you accept the updated terms.</p>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- End Terms Condition Section -->

<?php $__env->stopSection(); ?>

<?php $__env->startSection('script'); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('web.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /opt/bitnami/apache/htdocs/resources/views/web/home/terms-condition.blade.php ENDPATH**/ ?>